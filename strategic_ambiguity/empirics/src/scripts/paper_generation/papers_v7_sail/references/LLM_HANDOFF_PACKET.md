...
| Agent | Platform | Role | Focus | 강점 | 약점 |
|:------|:---------|:-----|:------|:-----|:-----|
| 🐢**정운** | ChatGPT | **Creator** | 초안 생성 | 창의적 초안, 깊은 서술 | 느림 (🐢) |
| 🐅**권준** | Claude Code | **Integrator** | **Semantics** | 논리 정밀화, 파일 통합, 코드 실행 | — |
| 🐣**나대용** | Claude Code | **Visualizer** | **Syntax** | figure, table, 시각적 포맷 일관성 | — |
| 🐙**김완** | Gemini | **Verifier** | 검증 | 실시간 웹 검색, 인용 검증 | — |

### 🔑 Semantics vs Syntax 역할 구분

| 구분 | 🐅권준 (Semantics) | 🐣나대용 (Syntax) |
|:-----|:------------------|:-----------------|
| **초점** | 의미/논리 | 형식/시각 |
| **담당** | 논증 흐름, 가설-검증 연결, 인용-근거 매칭 | 색상 코드, 표 포맷, Figure 레이아웃 |
| **질문** | "이 주장이 논리적인가?" | "이 Figure가 규칙에 맞는가?" |
| **출력** | Thesis_Master.md 본문 수정 | figures/, tables/ 시각 자료 |
| **기준** | CARE Framework (Crisp, Accessible, Reader-loving, Earned) | Color Convention (GREEN/BLUE/GRAY/GOLD) |
...
*v5.1 Updated: 2026-01-14 10:30 by 🐅권준 (Claude Code)*
*Major: Semantics/Syntax 역할 구분 명확화*
*권준 = Semantics (논리/의미), 나대용 = Syntax (형식/시각)*
*Workflow: 🐢정운(Creator) → 🐅권준(Semantics)/🐣나대용(Syntax) 병렬 → 🐙김완(Verifier)*
