...
def load_and_merge_data(industry_filter=None):
    ...
    # =========================================================================
    # CRITICAL METHODOLOGY: Rank-normalize V before computing M
    # =========================================================================
    #
    # WHY: Raw V values cluster at 80-90 (composite score), making raw
    # differences uninformative. 60% of companies have V_T == V_0 exactly.
    #
    # SOLUTION: Convert V to percentile ranks (0-100 uniform distribution)
    # - V_0_rank: percentile rank of V_0 within all 2021 observations
    # - V_T_rank: percentile rank of V_T within all 2025 observations
    # - D = V_T_rank - V_0_rank (relative position change)
    # - M = |D| (magnitude of relative position change)
    #
    # INTERPRETATION:
    # - M = 10 means company moved 10 percentile points in vagueness ranking
    ...
    # Preserve raw V values for reference
    panel['V_0_raw'] = panel['V_0'].copy()
    panel['V_T_raw'] = panel['V_T'].copy()

    # Rank-normalize: convert to percentile ranks (0-100)
    # Using 'average' method for ties to ensure smooth distribution
    panel['V_0'] = panel['V_0_raw'].rank(pct=True, method='average') * 100
    panel['V_T'] = panel['V_T_raw'].rank(pct=True, method='average') * 100

    print(f"\n  Rank Normalization Applied:")
    print(f"    V_0_raw range: [{panel['V_0_raw'].min():.1f}, {panel['V_0_raw'].max():.1f}]")
    print(f"    V_0_rank range: [{panel['V_0'].min():.1f}, {panel['V_0'].max():.1f}]")
    print(f"    V_T_raw range: [{panel['V_T_raw'].min():.1f}, {panel['V_T_raw'].max():.1f}]")
    print(f"    V_T_rank range: [{panel['V_T'].min():.1f}, {panel['V_T'].max():.1f}]")
    ...
    # Step 4: Classify mover_type (3 archetypes: stayer, zoom_in, zoom_out)
    ...
    # Clean up columns (include raw V values for reference)
    panel = panel[['company_id', 'V_0', 'V_T', 'V_0_raw', 'V_T_raw', 'D', 'M', 'E', 'G', 'L',
...
def save_thesis_panel(df, suffix=''):
    ...
    ds = xr.Dataset({
        ...
        'V_0_raw': ('company', df['V_0_raw'].values.astype(np.float64)),
        'V_T_raw': ('company', df['V_T_raw'].values.astype(np.float64)),
        ...
        'methodology_note': 'V_0 and V_T are RANK-NORMALIZED (percentile 0-100). V_0_raw/V_T_raw contain original values.',
        'rank_normalization': 'V values converted to percentile ranks to address clustering at 80-90 in raw scores',
        'n_companies': len(df),
