...
class DataValidator:
    ...
    def test_T1_3_V_values_match(self):
        """T1.3: V values are rank-normalized (0-100 percentile)."""
        # V_0 in panel is RANK-NORMALIZED (percentile), not raw V
        # Check that V_0 spans full percentile range
        valid = self.panel['V_0'].dropna()

        # After rank normalization, V_0 should span 0-100
        min_v = valid.min()
        max_v = valid.max()
        passed = (min_v >= 0) and (max_v <= 100) and (max_v - min_v > 90)

        self.add_result(
            'T1.3', 'V_0 is rank-normalized (percentile 0-100)', passed, 'CRITICAL',
            f"Range: [{min_v:.1f}, {max_v:.1f}]",
            expected="0-100 percentile range", actual=f"[{min_v:.1f}, {max_v:.1f}]"
        )
    ...
    def test_T3_1_stayer_classification(self):
        """T3.1: Stayer classification (NOT zoom_in AND NOT zoom_out)."""
        stayers = self.panel[self.panel['mover_type'] == 'stayer']
        # Stayer = M < 5 OR (M >= 5 AND |D| <= 10)
        # Equivalently: NOT (M >= 5 AND D < -10) AND NOT (M >= 5 AND D > 10)
        correct = ~(((stayers['M'] >= 5) & (stayers['D'] < -10)) |
                    ((stayers['M'] >= 5) & (stayers['D'] > 10))).any()

        self.add_result(
            'T3.1', 'Stayer classification (includes former horizontal)', correct, 'CRITICAL',
            f"All stayers are NOT zoom_in AND NOT zoom_out",
    ...
    def test_T3_4_partition_completeness(self):
        """T3.4: All companies classified into one of 3 archetypes."""
        ...
        self.add_result(
            'T3.4', 'Partition completeness (3 archetypes)', passed, 'CRITICAL',
    ...
    def test_T10_2_stayer_success_rate(self):
        """T10.2: Stayer success rate ~ 9.9%."""
        stayers = self.panel[self.panel['mover_type'] == 'stayer']
        rate = stayers['L'].mean() * 100
        expected = 9.9  # Updated from computed stats
        tolerance = 1.0
    ...
    def test_T10_4_stayer_proportion(self):
        """T10.4: Stayer proportion ~ 80.6% (with rank-normalized M)."""
        prop = (self.panel['mover_type'] == 'stayer').mean() * 100
        expected = 80.6  # Updated after rank normalization methodology
        tolerance = 2.0
