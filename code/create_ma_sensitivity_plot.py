...
OUTPUT_PATH = SCRIPT_DIR.parent / 'img' / 'AppC_Fig3_ma_sensitivity.png'
...
COLORS = {
    ...
    'dark_green': '#40A767',   # Dark green for ρ < 0 (consistent with Ch4_Fig2)
    'light_green': '#90EE90',  # Light green for ρ > 0 (consistent with Ch4_Fig2)
...
def create_visualization(results):
    ...
    for idx, (metric, label) in enumerate(zip(metrics, metric_labels)):
        ...
        if metric == 'rho_ER':
            ...
            # H3: Dark green for negative, light green for positive (consistent with Ch4_Fig2)
            bar_colors = [COLORS['dark_green'] if v < 0 else COLORS['light_green'] for v in vals]
            hypothesis_line = 0
            robust = all(v < 0 for v in vals)
